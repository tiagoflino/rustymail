...
graph TB
    ...
    subgraph Premium["Premium Crate (private)"]
        LLM[llama-cpp-2 Engine]
        AI_CMD[AI Commands]
        MODEL[Granite 3.2 2B GGUF]
    end

    subgraph Storage["Local Storage"]
        DB[(SQLite via sqlx)]
        CACHE[AI Summary Cache]
    end
...
- **AI**: llama-cpp-2 via private `rustymail-premium` crate. Metal (macOS), Vulkan (Linux/Windows), CUDA (NVIDIA). Granite 3.2 2B GGUF model. No data leaves the device.
- **Providers**: Gmail REST API, Microsoft Graph API, IMAP/SMTP with OAuth2, CalDAV
