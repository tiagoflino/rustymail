<script lang="ts">
  ...
  function resizeIframe(iframe: HTMLIFrameElement) {
    try {
      ...
      let s = resizeState.get(iframe);
      if (!s) { s = { lastH: 0, count: 0 }; resizeState.set(iframe, s); }
      if (h > 0 && h !== s.lastH && s.count < 20) {
        s.lastH = h;
        s.count++;
