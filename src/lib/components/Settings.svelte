<script lang="ts">
  ...
  import { iconClose, iconUser, iconPlus, iconCheck } from '$lib/components/icons';

  interface SettingsProps {
    show: boolean;
    accounts: Array<{id: string; email: string; display_name: string; avatar_url: string; is_active: boolean}>;
    onclose: () => void;
  ...
  let { show = $bindable(false), accounts = [], onclose = () => {}, onAccountSwitch = () => {}, onAccountAdd = () => {}, onAccountRemove = () => {} }: SettingsProps = $props();
  ...
  async function loadSettings() {
    try {
      const entries: Array<{key: string; value: string}> = await invoke('get_settings');
      settings = {};
      ...
      <div class="settings-content">
        {#if activeTab === 'accounts'}
          <div class="section">
            ...
            <p class="section-desc">Manage your Google accounts linked to Rustymail.</p>
            <div class="account-list">
              {#each accounts as account}
                <div class="account-row">
                  <div class="account-avatar">
                    {#if account.avatar_url}
                      <img src={account.avatar_url} alt="" class="avatar-img" referrerpolicy="no-referrer" />
                    {:else}
                  ...
                  <div class="account-actions">
                    {#if account.is_active}
                      <span class="active-badge">{@html iconCheck} Active</span>
                    {:else}
                      <button class="btn-sm" onclick={() => onAccountSwitch(account.id)}>Switch</button>
                    {/if}
                    <button class="btn-sm btn-danger" onclick={() => onAccountRemove(account.id)}>Remove</button>
                  </div>
                </div>
              {/each}
            </div>
        ...
        {:else if activeTab === 'sync'}
          <div class="section">
            ...
            <p class="section-desc">Control how Rustymail fetches and stores your email data.</p>
            ...
            <div class="setting-group">
              <div class="setting-label">
                ...
                <span class="setting-hint">Max threads fetched per sync cycle</span>
              </div>
              <div class="slider-row">
                <input type="range" class="range-slider" min="0" max={threadStops.length - 1} step="1"
                  value={threadToSlider(settings.max_threads_sync || '100')}
                  oninput={(e) => saveSetting('max_threads_sync', sliderToThread(parseInt(e.currentTarget.value)))} />
                <span class="slider-value">{settings.max_threads_sync || '100'}</span>
              </div>
            </div>
            ...
            <div class="setting-group">
              <div class="setting-label">
                ...
                <span class="setting-hint">Maximum disk space for cached emails</span>
              </div>
              <div class="slider-row">
                <input type="range" class="range-slider" min="0" max={cacheStops.length - 1} step="1"
                  value={cacheToSlider(settings.max_cache_mb || '500')}
                  oninput={(e) => saveSetting('max_cache_mb', sliderToCache(parseInt(e.currentTarget.value)))} />
                <span class="slider-value">{cacheLabel(settings.max_cache_mb || '500')}</span>
              </div>
            </div>
          </div>
        ...
        {:else if activeTab === 'reading'}
          <div class="section">
            ...
            <p class="section-desc">Customize how you read and navigate emails.</p>

            <div class="setting-group">
              <div class="setting-label">
                <span class="setting-name">Default Mailbox</span>
                <span class="setting-hint">Which folder to open on launch</span>
              </div>
              <div class="option-group">
                {#each [['INBOX', 'Inbox'], ['STARRED', 'Starred'], ['SENT', 'Sent']] as [val, label]}
                  <button class="option-btn {(settings.default_mailbox || 'INBOX') === val ? 'selected' : ''}" onclick={() => saveSetting('default_mailbox', val)}>{label}</button>
                {/each}
              </div>
            </div>
            ...
            <div class="setting-group">
              <div class="setting-label">
                ...
                <span class="setting-hint">Delay before marking opened emails as read</span>
              </div>
              <div class="option-group">
        ...
        {:else if activeTab === 'notifications'}
          <div class="section">
            ...
            <p class="section-desc">Choose when and how Rustymail notifies you.</p>
            <label class="toggle-row">
              <div class="toggle-label">
                <span class="setting-name">Desktop Notifications</span>
                <span class="setting-hint">Show alerts for new messages</span>
              </div>
              <input type="checkbox" class="toggle" checked={settings.notifications_enabled === 'true'} onchange={(e) => saveSetting('notifications_enabled', e.currentTarget.checked ? 'true' : 'false')} />
            </label>
            <label class="toggle-row">
              <div class="toggle-label">
                <span class="setting-name">Notification Sounds</span>
                <span class="setting-hint">Play a sound when messages arrive</span>
              </div>
              <input type="checkbox" class="toggle" checked={settings.notifications_sound === 'true'} onchange={(e) => saveSetting('notifications_sound', e.currentTarget.checked ? 'true' : 'false')} />
            </label>
          </div>
        ...
        {:else if activeTab === 'about'}
          <div class="section about-section">
            ...
            <p class="about-desc">A fast, private, cross-platform Gmail client built with Rust and Tauri.</p>
...
<style>
  ...
  .setting-hint { font-size: 11px; color: var(--text-secondary); }
  ...
  .option-btn:hover { background: var(--sidebar-hover); }
  .option-btn.selected {
  ...
  .toggle-row.compact { padding: 6px 0; border-bottom: none; justify-content: flex-start; gap: 8px; }
  ...
  .account-row:hover { background: var(--sidebar-hover); }

  .account-avatar { width: 36px; height: 36px; flex-shrink: 0; }
  .avatar-img { width: 36px; height: 36px; border-radius: 50%; object-fit: cover; }
  .avatar-placeholder { 
    width: 36px; height: 36px; border-radius: 50%; 
    background: var(--sidebar-hover); display: flex; 
    align-items: center; justify-content: center; color: var(--text-secondary);
  }

  .account-info { flex: 1; display: flex; flex-direction: column; overflow: hidden; }
  .account-name { font-size: 13px; font-weight: 500; }
  .account-email { font-size: 11px; color: var(--text-secondary); }
  ...
  .btn-sm {
    padding: 4px 10px; font-size: 11px; border: 1px solid var(--border-color);
    border-radius: 5px; background: transparent; color: var(--text-primary);
    cursor: pointer; font-family: var(--font-family); transition: all 0.1s;
  }
  ...
  .shortcut-row {
    ...
    padding: 8px 0; font-size: 13px; border-bottom: 1px solid var(--border-color);
  }
  .shortcut-row:last-child { border-bottom: none; }
  kbd {
    background: var(--sidebar-hover); padding: 3px 10px; border-radius: 5px;
    font-size: 12px; font-family: -apple-system, monospace; border: 1px solid var(--border-color);
  ...
  .about-name { font-size: 20px; font-weight: 600; color: var(--text-primary); }
  .about-version { font-size: 12px; color: var(--text-secondary); margin-top: 4px; }
  .about-desc { font-size: 13px; color: var(--text-secondary); margin-top: 16px; max-width: 320px; line-height: 1.5; }
  .about-links { margin-top: 24px; }
  .about-link { font-size: 12px; color: var(--text-secondary); }
</style>
